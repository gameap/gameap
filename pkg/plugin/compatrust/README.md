...
| `stubs_head_test.go` | `HEAD` only | Stub methods for host-module RPCs added after the latest release (nodefs hash/archive, nodes write/setup-key) — dropped on every tagged leg, where the interface does not have them |
