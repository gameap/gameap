...
type Config struct {
	...
	// Plugins configures the plugin subsystem: which plugins load at
	// startup, the wazero runtime that executes them and the capabilities
	// they reach through the host libraries (HTTP fetch, file IO, etc.).
	// Capability defaults are intentionally strict — a malicious or
	// compromised plugin must not be able to pivot from the panel process
	// into private networks, cloud metadata endpoints, or services that
	// trust the panel's source IP.
	Plugins struct {
		...
		// Store is the catalog the panel installs and updates plugins from.
		Store struct {
			URL        string `env:"PLUGINS_STORE_URL" envDefault:"https://plugins.gameap.dev/api"`
			LicenseKey string `env:"PLUGINS_STORE_LICENSE_KEY" envDefault:""`
		}
		...
		HTTP struct {
			...
			BlockPrivateIPs bool `env:"PLUGINS_HTTP_BLOCK_PRIVATE_IPS" envDefault:"true"`
			...
			AllowedSchemes []string `env:"PLUGINS_HTTP_ALLOWED_SCHEMES" envSeparator:"," envDefault:"https"`
			...
			AllowedHosts []string `env:"PLUGINS_HTTP_ALLOWED_HOSTS" envSeparator:"," envDefault:""`
			...
			MaxTimeout time.Duration `env:"PLUGINS_HTTP_MAX_TIMEOUT" envDefault:"30s"`
			...
			// into RFC1918.
			MaxRedirects int `env:"PLUGINS_HTTP_MAX_REDIRECTS" envDefault:"5"`
		...
		Scheduler struct {
			...
			MinInterval time.Duration `env:"PLUGINS_SCHEDULER_MIN_INTERVAL" envDefault:"1s"`

			// MaxTasksPerPlugin caps registrations per plugin.
			MaxTasksPerPlugin int `env:"PLUGINS_SCHEDULER_MAX_TASKS_PER_PLUGIN" envDefault:"32"`

			// CallTimeout bounds one handler call when the task does not
			// set its own timeout; matches the async event budget.
			CallTimeout time.Duration `env:"PLUGINS_SCHEDULER_CALL_TIMEOUT" envDefault:"60s"`

			// MaxCallTimeout is the ceiling for per-task timeout overrides.
			MaxCallTimeout time.Duration `env:"PLUGINS_SCHEDULER_MAX_CALL_TIMEOUT" envDefault:"5m"`

			// MaxRetries, MaxRetryDelay and MaxJitter cap the per-task
			// retry policy a plugin may request.
			MaxRetries    int           `env:"PLUGINS_SCHEDULER_MAX_RETRIES" envDefault:"10"`
			MaxRetryDelay time.Duration `env:"PLUGINS_SCHEDULER_MAX_RETRY_DELAY" envDefault:"10m"`
			MaxJitter     time.Duration `env:"PLUGINS_SCHEDULER_MAX_JITTER" envDefault:"30s"`

			// RefreshInterval is how often task definitions are re-read
			// from the database to pick up registrations made by other
			// panel instances.
			RefreshInterval time.Duration `env:"PLUGINS_SCHEDULER_REFRESH_INTERVAL" envDefault:"30s"`
		}
		...
		Secrets struct {
			...
			MaxKeysPerPlugin int `env:"PLUGINS_SECRETS_MAX_KEYS_PER_PLUGIN" envDefault:"64"`

			// MaxValueBytes caps the plaintext size of a single secret.
			MaxValue ByteSize `env:"PLUGINS_SECRETS_MAX_VALUE" envDefault:"8K"`
			...
			RequireEncryption bool `env:"PLUGINS_SECRETS_REQUIRE_ENCRYPTION" envDefault:"true"`
		}
		...
		SSH struct {
			...
			Enabled bool `env:"PLUGINS_SSH_ENABLED" envDefault:"false"`
			...
			BlockPrivateIPs bool `env:"PLUGINS_SSH_BLOCK_PRIVATE_IPS" envDefault:"true"`
			...
			AllowedHosts []string `env:"PLUGINS_SSH_ALLOWED_HOSTS" envSeparator:"," envDefault:""`
			...
			// or its fingerprint.
			AllowAcceptAnyHostKey bool `env:"PLUGINS_SSH_ALLOW_ACCEPT_ANY_HOST_KEY" envDefault:"true"`

			// MaxConnections caps concurrent SSH connections per plugin.
			MaxConnections int `env:"PLUGINS_SSH_MAX_CONNECTIONS" envDefault:"8"`

			// MaxOperations caps concurrently running commands per plugin.
			MaxOperations int `env:"PLUGINS_SSH_MAX_OPERATIONS" envDefault:"16"`

			// ConnectTimeout bounds the dial, handshake and authentication
			// together, and is also the ceiling for a plugin's own value.
			ConnectTimeout time.Duration `env:"PLUGINS_SSH_CONNECT_TIMEOUT" envDefault:"30s"`

			// MaxExecTimeout is the ceiling for a single remote command.
			MaxExecTimeout time.Duration `env:"PLUGINS_SSH_MAX_EXEC_TIMEOUT" envDefault:"30m"`

			// IdleTimeout closes a connection nothing has run on for this long.
			IdleTimeout time.Duration `env:"PLUGINS_SSH_IDLE_TIMEOUT" envDefault:"10m"`

			// MaxOutputBytes caps captured stdout and stderr per command; the
			// head is kept and the stream is reported as truncated.
			MaxOutputBytes int `env:"PLUGINS_SSH_MAX_OUTPUT_BYTES" envDefault:"1048576"`

			// MaxStdinBytes caps what a plugin may pipe into a command, which
			// is how install scripts are delivered.
			MaxStdinBytes int `env:"PLUGINS_SSH_MAX_STDIN_BYTES" envDefault:"1048576"`
			...
			OperationRetention time.Duration `env:"PLUGINS_SSH_OPERATION_RETENTION" envDefault:"10m"`
		...
		Net struct {
			...
			Enabled bool `env:"PLUGINS_NET_ENABLED" envDefault:"true"`
			...
			BlockPrivateIPs bool `env:"PLUGINS_NET_BLOCK_PRIVATE_IPS" envDefault:"false"`
			...
			AllowedHosts []string `env:"PLUGINS_NET_ALLOWED_HOSTS" envSeparator:"," envDefault:""`

			// MaxTimeoutSeconds caps the overall duration of a single plugin
			// protocol operation (dial plus all reads and writes).
			MaxTimeout time.Duration `env:"PLUGINS_NET_MAX_TIMEOUT" envDefault:"10s"`

			// ReadBufferBytes caps a single Recv a plugin may request.
			ReadBuffer ByteSize `env:"PLUGINS_NET_READ_BUFFER" envDefault:"64K"`

			// MaxConnections caps simultaneously open connections per plugin.
			MaxConnections int `env:"PLUGINS_NET_MAX_CONNECTIONS" envDefault:"8"`
		}
		...
		Runtime struct {
			...
			MaxMemory ByteSize `env:"PLUGINS_RUNTIME_MAX_MEMORY" envDefault:"256M"`
		...
		Permissions struct {
			...
			Enforce bool `env:"PLUGINS_PERMISSIONS_ENFORCE" envDefault:"false"`
			...
			CacheTTL time.Duration `env:"PLUGINS_PERMISSIONS_CACHE_TTL" envDefault:"30s"`
		}
		...
		Recovery struct {
			Enabled      bool          `env:"PLUGINS_RECOVERY_ENABLED" envDefault:"true"`
			InitialDelay time.Duration `env:"PLUGINS_RECOVERY_INITIAL_DELAY" envDefault:"30s"`
			MaxDelay     time.Duration `env:"PLUGINS_RECOVERY_MAX_DELAY" envDefault:"10m"`
			MaxAttempts  int           `env:"PLUGINS_RECOVERY_MAX_ATTEMPTS" envDefault:"5"`
		}
		...
		Sync struct {
			...
			Disabled bool `env:"PLUGINS_SYNC_DISABLED" envDefault:"false"`
			// RefreshInterval bounds how long a lost hint can leave an
			// instance stale.
			RefreshInterval time.Duration `env:"PLUGINS_SYNC_REFRESH_INTERVAL" envDefault:"60s"`
			// MinBackoff / MaxBackoff bound the retry delay after a plugin
			// failed to load on this instance.
			MinBackoff time.Duration `env:"PLUGINS_SYNC_MIN_BACKOFF" envDefault:"15s"`
			MaxBackoff time.Duration `env:"PLUGINS_SYNC_MAX_BACKOFF" envDefault:"15m"`
		}
		...
		NodeFS struct {
			...
			MaxInline ByteSize `env:"PLUGINS_NODEFS_MAX_INLINE" envDefault:"32M"`
			...
			PathPolicy string `env:"PLUGINS_NODEFS_PATH_POLICY" envDefault:"unrestricted"`
			// AllowedPaths are additional absolute roots accepted in the
			// restricted modes, on every node.
			AllowedPaths []string `env:"PLUGINS_NODEFS_ALLOWED_PATHS" envSeparator:"," envDefault:""`
		}
		...
		Storage struct {
			...
			MaxKeysPerPlugin int `env:"PLUGINS_STORAGE_MAX_KEYS_PER_PLUGIN" envDefault:"10000"`

			// MaxValueBytes caps a single payload.
			MaxValue ByteSize `env:"PLUGINS_STORAGE_MAX_VALUE" envDefault:"1M"`

			// MaxTotalBytes caps the sum of all payloads one plugin owns.
			MaxTotal ByteSize `env:"PLUGINS_STORAGE_MAX_TOTAL" envDefault:"64M"`
		}
		...
		Cache struct {
			...
			MaxValue ByteSize `env:"PLUGINS_CACHE_MAX_VALUE" envDefault:"1M"`
		}
		...
		RateLimit struct {
			...
			NodeCmd struct {
				RPS   float64 `env:"PLUGINS_RATELIMIT_NODECMD_RPS" envDefault:"5"`
				Burst int     `env:"PLUGINS_RATELIMIT_NODECMD_BURST" envDefault:"20"`
			}

			// ServerControl: gameap-servercontrol, daemon task creation,
			// server and server-setting writes.
			ServerControl struct {
				RPS   float64 `env:"PLUGINS_RATELIMIT_SERVERCONTROL_RPS" envDefault:"5"`
				Burst int     `env:"PLUGINS_RATELIMIT_SERVERCONTROL_BURST" envDefault:"20"`
			}

			// NodeFS: every gameap-nodefs operation.
			NodeFS struct {
				RPS   float64 `env:"PLUGINS_RATELIMIT_NODEFS_RPS" envDefault:"50"`
				Burst int     `env:"PLUGINS_RATELIMIT_NODEFS_BURST" envDefault:"200"`
			}

			// HTTP: gameap-http outbound requests.
			HTTP struct {
				RPS   float64 `env:"PLUGINS_RATELIMIT_HTTP_RPS" envDefault:"20"`
				Burst int     `env:"PLUGINS_RATELIMIT_HTTP_BURST" envDefault:"50"`
			}

			// RBAC: gameap-rbac role and ability writes.
			RBAC struct {
				RPS   float64 `env:"PLUGINS_RATELIMIT_RBAC_RPS" envDefault:"10"`
				Burst int     `env:"PLUGINS_RATELIMIT_RBAC_BURST" envDefault:"50"`
			}

			// SSH: every gameap-ssh call. The budget has to cover polling a
			// running command with GetExecOperation, not just the connects.
			SSH struct {
				RPS   float64 `env:"PLUGINS_RATELIMIT_SSH_RPS" envDefault:"20"`
