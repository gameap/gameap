...
A server with `Blocked` set is suspended (`server_suspension.go`): stopped, and refused start, restart, update, install and reinstall for everyone, administrators and tokens included. Enforcement points ask `IsSuspended()`, not the field. `Suspend`/`Unsuspend` also keep the record of when and why in the `suspension` metadata key, which only the panel writes: `ReplaceMetadata` carries it over when an update replaces the metadata, and `PublicMetadata` leaves it out of API responses, which show it as `suspension`. `internal/services/serversuspension` saves a suspension, pushes it to the daemon and stops the server; `servercontrol` refuses the operations (`ErrServerBlocked`, 403).
