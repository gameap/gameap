...
func (h *Handler) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	...
	stopTaskID, err := h.suspension.Suspend(ctx, &servers[0], in.Reason)
	if err != nil {
		h.responder.WriteError(ctx, rw, errors.WithMessage(err, "failed to suspend server"))

		return
	}

	h.responder.Write(ctx, rw, newSuspendResponse(stopTaskID))
}
