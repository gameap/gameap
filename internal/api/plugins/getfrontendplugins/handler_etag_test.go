...
func TestHandler_ServeHTTP_ConditionalGet(t *testing.T) {
	...
	provider := &mockPluginProvider{plugins: []*plugin.LoadedPlugin{
		{FrontendBundle: []byte("console.log(1)")},
	}}
	...
	t.Run("etag_changes_with_the_bundle", func(t *testing.T) {
		t.Parallel()
		other := getfrontendplugins.NewHandler(&mockPluginProvider{plugins: []*plugin.LoadedPlugin{
			{FrontendBundle: []byte("console.log(1) changed")},
		}})
	...
	t.Run("mismatch_returns_full_body", func(t *testing.T) {
		...
		assert.Contains(t, recorder.Body.String(), "console.log(1)")
	})
}
