...
// The file-name field is attacker-controlled; without strict filename
// validation a caller could traverse out of the server data directory
// (path/filename traversal) and read or overwrite objects belonging to
// other tenants. These tests assert the NAME field is validated with
...
// handler rejects names that could escape the per-server directory while the
// directory Path field still accepts a legitimate relative path.
