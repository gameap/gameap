...
//   - API8:2023 Security Misconfiguration — a file write that succeeds
...
func newUpdateFileAuditServer() *domain.Server {
	now := time.Now()
...
// TestHandler_Audit_SuccessfulWriteIsRecorded covers OWASP API8:2023. A
// successful file write must emit exactly one file.write event with outcome
...
func TestHandler_Audit_SuccessfulWriteIsRecorded(t *testing.T) {
	...
	require.NoError(t, serverRepo.Save(context.Background(), newUpdateFileAuditServer()))
	serverRepo.AddUserServer(1, 1)
	allowUserFilesAbility(t, rbacRepo, 1, 1)
	node := testNode
	require.NoError(t, nodeRepo.Save(context.Background(), &node))
	...
	part, err := writer.CreateFormFile("file", "eula.txt")
	require.NoError(t, err)
	_, err = part.Write([]byte("eula=true\ntest=yes"))
	require.NoError(t, err)
	require.NoError(t, writer.Close())

	req := httptest.NewRequest(http.MethodPost, "/api/file-manager/1/update-file", body)
	req.Header.Set("Content-Type", writer.FormDataContentType())
	...
	// ASSERT
	require.Equal(t, http.StatusOK, w.Code, "file write must succeed; body=%s", w.Body.String())

	events := recorder.snapshot()
	require.Equal(t, 1, countEvents(events, audit.EventFileWrite),
		"exactly one file-write event must be emitted per successful write")

	ev, ok := findEvent(events, audit.EventFileWrite)
	require.True(t, ok, "a successful write must leave a file.write audit event")
	...
	assert.Equal(t, "write", ev.Action)
...
// TestHandler_Audit_DeniedWriteDoesNotEmitFileWrite covers OWASP API8:2023.
...
func TestHandler_Audit_DeniedWriteDoesNotEmitFileWrite(t *testing.T) {
	...
	assert.Equal(t, 0, countEvents(recorder.snapshot(), audit.EventFileWrite),
