...
// describes, fetching it from the store when it is not.
//
...
func (s *Service) ensureFile(ctx context.Context, row *domain.Plugin, filename string) error {
	pluginPath := path.Join(s.pluginsDir, filename)
...
func (s *Service) download(ctx context.Context, row *domain.Plugin, storeID, pluginPath string) error {
	...
	if err != nil {
		return errors.WithMessage(err, "failed to download plugin file")
	}
	...
	if err := s.files.Write(ctx, pluginPath, data); err != nil {
		return errors.WithMessage(err, "failed to write plugin file")
	}
