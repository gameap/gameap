...
// apply lock before the manager's — a host library reaching back here would
// invert that order.
package pluginsync
...
const (
	...
	defaultLoadTimeout        = 90 * time.Second
...
// SyncState is what the admin API reports for one plugin on this instance.
type SyncState string

const (
	// SyncStateInSync means the runtime matches the database row.
	SyncStateInSync SyncState = "in_sync"
	// SyncStateRetrying means the last attempt failed and another is scheduled.
	SyncStateRetrying SyncState = "retrying"
	// SyncStateOrphan means the module is loaded but no database row explains
	// it. It is left alone until this instance observes the row disappear.
	SyncStateOrphan SyncState = "orphan"
	// SyncStatePending means the database wants the plugin active and this
...
type Options struct {
	...
	// holds the download lock. Contention is not failure.
	ContentionBackoff time.Duration
	// LoadTimeout must exceed the manager's own module initialization budget,
	// otherwise a slow but healthy plugin can never load.
...
// pluginState is what this instance believes it applied for one plugin.
...
type Service struct {
	...
	// apply lock, which both go through.
	runMu sync.Mutex
...
//
// The first pass is synchronous on purpose: it records the fingerprint of every
// plugin the loader has just brought up. Without that baseline a change made by
// a peer moments later would be indistinguishable from the state this instance
// started with.
...
//
// Like Notify, Forget and Snapshot, it tolerates a nil receiver: the container
// hands out a nil *Service when plugin sync is switched off, and that value
// still reaches the handlers through an interface, where a plain nil check
// would not catch it.
func (s *Service) Kick() {
	if s == nil {
		return
...
// Notify publishes a hint that a plugin changed. It is best effort: the hint
// only shortens the delay before peers notice, so a publish failure degrades
// latency, never correctness.
func (s *Service) Notify(ctx context.Context, pluginID domain.Uint64ID, reason string) {
	if s == nil || s.bus == nil {
		return
	}
...
// Forget drops what this instance recorded for a plugin. Handlers call it after
// applying a change themselves, so the next pass adopts their work instead of
// tearing the module down and rebuilding it to reach the same place.
func (s *Service) Forget(pluginID domain.Uint64ID) {
	if s == nil {
		return
	}
...
func (s *Service) Snapshot() map[domain.Uint64ID]Status {
	...
	for id, st := range s.state {
		...
		// lastErr and not just failures: download contention schedules a retry
		// without counting as a failure, and reporting that as in_sync would
		// claim the plugin is running when it is not.
		if st.failures > 0 || st.lastErr != "" {
			status.State = SyncStateRetrying
		}
