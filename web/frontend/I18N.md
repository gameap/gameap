...
> **Plugins** may contribute additional `/lang/*.json` files via their `GetAssets` capability.
> Plugin files are layered **above** the built-in translations (a plugin `en.json` would shadow
> the core one wholesale, so plugins normally add a new locale such as `es.json` instead). See
> `pkg/plugin/README.md` → "Contributing translation & frontend files".
