...
func normalizeUserIdentifiers(ctx context.Context, tx *sql.Tx) error {
	...
	err = applyIdentifierFolding(ctx, tx, `UPDATE users SET login = $1 WHERE id = $2`, logins)
	if err != nil {
		return err
	}

	return applyIdentifierFolding(ctx, tx, `UPDATE users SET email = $1 WHERE id = $2`, emails)
}
