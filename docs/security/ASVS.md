...
| 14.1.3 | Dependencies up to date and patched | ✅ Met | `go.mod`/`go.sum` version-pinned; `.github/workflows/vuln-scan.yaml` (2026-05-28, T-10 closed) runs `govulncheck@v1.1.4` against source + binary modes weekly + on push to main + manual dispatch, auto-files / auto-closes a GitHub issue per failing mode, and validates the manual-dispatch version override against a semver pattern to defend against workflow-injection via that input. |
